...
class HiggsGenVarsProducer(Module):
    ...
    def runModule(self, df, values):
        ...
        df = df.Define(
            "Higgs_mask",
            "GenPart_pdgId == 25 && bitExpr(GenPart_statusFlags, 13)"
        )
        
        df = df.Define(
            "higgsGenPt",
            "HiggsSelect(Higgs_mask, GenPart_pt)"
        )

        df = df.Define(
            "higgsGenEta",
            "HiggsSelect(Higgs_mask, GenPart_eta)"
        )

        df = df.Define(
            "higgsGenPhi",
            "HiggsSelect(Higgs_mask, GenPart_phi)"
        )

        df = df.Define(
            "higgsGenMass",
            "HiggsSelect(Higgs_mask, GenPart_mass)"
        )
