...
class runDependentPuWRun3(Module):
    def __init__(
            self,
            era="Full2022EEv11",
            files = []
    ):
        super().__init__("runDependentPuWRun3")
    ...
    def runModule(self, df, values):
        ...
        for histName in self.PUWeightCfg['targethist']:
            ...
            for rpr in self.PUWeightCfg['targetfiles'] : 
                ...
                if first:
                    ...
                    ROOT.gROOT.ProcessLine(
                        f"""TFile* PUFile_{targeth['beginRP']} = TFile::Open("{inputFile}");"""
                    )

                    first = False
                
                s = f"""
                TH1D* PUProfile_{targeth['beginRP']}_{histName} = PUFile_{targeth['beginRP']}->Get<TH1D>("{histName}");
                """
                ...
                if self.PUWeightCfg['doSysVar']:                
                    s = f"""
                    TH1D* PUProfile_{targeth['beginRP']}_{histName}_plus =  PUFile_{targeth['beginRP']}->Get<TH1D>("{histName}_plus");
                    TH1D* PUProfile_{targeth['beginRP']}_{histName}_minus =  PUFile_{targeth['beginRP']}->Get<TH1D>("{histName}_minus");
                    """            
                    ...
                    if (runP>="""+str(targeth['beginRP'])+""" && runP<="""+str(targeth['endRP'])+"""){
                       targethist       = PUProfile_"""+str(targeth['beginRP'])+"""_"""+histName+""";
                       targethist_plus  = PUProfile_"""+str(targeth['beginRP'])+"""_"""+histName+"""_plus;
                       targethist_minus = PUProfile_"""+str(targeth['beginRP'])+"""_"""+histName+"""_minus;
                    }
                    """
                else:
                    which_runP = which_runP + """                                                                                                                                                              
            ...
            if self.PUWeightCfg['srcfile'] != "auto" :
                ...
                TH1* hist = SRCFile->Get<TH1*>("{histName}");                                                                                                                                     
                """
            else:
                fixLargeWeights = False
                
                myFile = ROOT.TFile.Open(inputFile, "READ")
                myHist = myFile.Get(histName)
                myModel = ROOT.RDF.TH1DModel(myHist)
                hist = df.Histo1D(myModel, histName)
            ...
            ROOT.gInterpreter.Declare(
                """
                float computeWeights_"""+histName+"""(int nvertex, TH1D* targethist, bool norm, bool verbose_, bool fixLargeWgts, std::vector<float> refvals_, std::vector<float> targetvals_){
                ...
                ROOT::RVecF puWeights_"""+histName+"""(int nvertex, int runP, bool doSyst, bool fixLargeWeights, bool norm, bool verbose){
                    ...
                    if (hist.GetNcells()!=targethist->GetNcells()){
                        ...
                        results = computeWeights_"""+histName+"""(nvertex, targethist, norm, verbose, fixLargeWeights, refvals_, targetvals_);
                        if (doSyst){
                            results_plus = computeWeights_"""+histName+"""(nvertex, targethist, norm, verbose, fixLargeWeights, refvals_, targetvals_plus);
                            results_minus = computeWeights_"""+histName+"""(nvertex, targethist, norm, verbose, fixLargeWeights, refvals_, targetvals_minus);
                        }
                    }
                    if (!doSyst){
            ...
            df = df.Define(
                "puWeight_"+histName,
                f"puWeights_{histName}("+histName+", run_period, "+str(doSysVar).lower()+", "+str(fixLargeWeights).lower()+", "+str(norm).lower()+", "+str(verbose).lower()+")[0]"
            )
            
            df = df.Define(
                "puWeight_"+histName+"Up",
                f"puWeights_{histName}("+histName+", run_period, "+str(doSysVar).lower()+", "+str(fixLargeWeights).lower()+", "+str(norm).lower()+", "+str(verbose).lower()+")[1]"
            )
            
            df = df.Define(
                "puWeight_"+histName+"Down",
                f"puWeights_{histName}("+histName+", run_period, "+str(doSysVar).lower()+", "+str(fixLargeWeights).lower()+", "+str(norm).lower()+", "+str(verbose).lower()+")[2]"
            )  
            
            variablesToAdd.append("puWeight_"+histName)
            variablesToAddUp.append("puWeight_"+histName+"Up")
            variablesToAddDown.append("puWeight_"+histName+"Down")            
        ...
        '''
        s = "(" + "+".join(variablesToAdd) + ")/" + str(len(variablesToAdd))

        var_up = ["(("+s+"Up-"+s+")/"+s+")*(("+s+"Up-"+s+")/"+s+")" for s in variablesToAdd]
        s_up = "sqrt(" + "+".join(var_up) + ")*" + name

        var_down = ["(("+s+"-"+s+"Down)/"+s+")*(("+s+"-"+s+"Down)/"+s+")" for s in variablesToAdd]
        s_down = "sqrt(" + "+".join(var_down) + ")*" + name
