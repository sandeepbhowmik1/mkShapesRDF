...
class PromptParticlesGenVarsProducer(Module):
    ...
    def runModule(self, df, values):
        ...
        df = df.Define(
            "PhotonGen_mask",
            "GenPart_pdgId == 22 and GenPart_status == 1"
        )

        df = df.Define(
            "GenPart_fromHardProcess",
            "bitExpr(GenPart_statusFlags, 8)"
        )

        df = df.Define(
            "GenPart_isDirectHadronDecayProduct",
            "bitExpr(GenPart_statusFlags, 6)"
        )

        df = df.Define(
            "GenPart_isDirectPromptTauDecayProduct",
            "bitExpr(GenPart_statusFlags, 5)"
        )

        df = df.Define(
            "GenPart_isPrompt",
            "bitExpr(GenPart_statusFlags, 0)"
        )

        df = df.Define(
            "GenPart_isTauDecayProduct",
            "bitExpr(GenPart_statusFlags, 3)"
        )
        ...
        df = df.Define(
            ...
            "Take(GenPart_status, GenPart_genPartIdxMother)"
        )
        ...
        for pType in particleTypes:
            for prop in branches:
                ...
                df = df.Redefine(f"{pType}_{prop}", f"Take({pType}_{prop}, {pType}_sorting)")
