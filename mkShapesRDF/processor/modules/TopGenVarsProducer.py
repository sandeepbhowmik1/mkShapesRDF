...
class TopGenVarsProducer(Module):
    ...
    def runModule(self, df, values):
        ...
        df = df.Define(
            "top_mask",
            "(GenPart_pdgId == 6 && bitExpr(GenPart_statusFlags, 13))"
        )
        ...
        df = df.Define(
            "topGenPt",
            "TopSelect(top_mask, GenPart_pt)"
        )

        df = df.Define(
            "topGenEta",
            "TopSelect(top_mask, GenPart_eta)"
        )

        df = df.Define(
            "topGenPhi",
            "TopSelect(top_mask, GenPart_phi)"
        )

        df = df.Define(
            "topGenMass",
            "TopSelect(top_mask, GenPart_mass)"
        )

        df = df.Define(
            "antitopGenPt",
            "TopSelect(antitop_mask, GenPart_pt)"
            )

        df = df.Define(
            "antitopGenEta",
            "TopSelect(antitop_mask, GenPart_eta)"
        )

        df = df.Define(
            "antitopGenPhi",
            "TopSelect(antitop_mask, GenPart_phi)"
        )

        df = df.Define(
            "antitopGenMass",
            "TopSelect(antitop_mask, GenPart_mass)"
        )
